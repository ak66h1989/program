...
for i in os.listdir(path+'till20153/'):
    try:
        ...
        for j in os.listdir():
            ...
            # print(df)
            l.append(df)
        df1 = reduce(mymerge, l).drop_duplicates(subset=['年', '季', '公司代號']).sort_values(['年', '季'], ascending=[1, 1]).reset_index(drop=True)
...
# for i in os.listdir(path+'till20153/'):
#     try:
#         df = read_csv(i, encoding = 'big5').rename(columns = {'證券代號':'公司代號'})
