    ...
    for report in ['A']:
        try:
            ...
            if h == ['證券代號', '年季', '\u3000 核閱或查核日期']:
                ...
                conn.commit()
                if report == 'A':
                    c.execute('INSERT INTO `REPORT_ID_A` VALUES (?,?,?)', i)
...
df = read_sql_query("SELECT * from `會計師查核報告`", conn)
