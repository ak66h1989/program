...
col = {'資產總計': '資產總額', '負債總計': '負債總額', '少數股權': '非控制權益', '股東權益其他調整項目合計': '其他權益', '庫藏股票(自98年第4季起併入「其他項目」表達)': '庫藏股票', '股東權益總計': '權益總額', '預收股款(股東權益項下)之約當發行股數(單位:股)':'預收股款（權益項下）之約當發行股數（單位：股）','母公司暨子公司所持有之母公司庫藏股股數(單位:股)':'母公司暨子公司所持有之母公司庫藏股股數（單位：股）', '每股淨值':'每股參考淨值'}
df0 = read_sql_query("SELECT * from `資產負債表-一般業`", conn)
del df0['Unnamed: 21']
del df0['Unnamed: 22']
df1 = read_sql_query("SELECT * from `ifrs前-合併資產負債表-一般業`", conn)
df1 = df1.rename(columns= col)
df = mymerge(df0, df1)
# list(df0)
# list(df1)
# list(df)
# [x for x in list(df0) if x not in list(df1)]
# [x for x in list(df1) if x not in list(df0)]
